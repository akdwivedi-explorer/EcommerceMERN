...
export const getAllCartItems = async (req, res, next) => {
  try {
    const userJWT = req.user; // Ensure `req.user` is populated by authentication middleware
    const user = await User.findById(userJWT.id).populate({
      path: "cart.product", // Populate the product field in cart
      model: "Products", // Ensure "Products" is a valid registered model
    });
    ...
    return res.status(200).json(user.cart); // Return populated cart
  } catch (err) {
...
export const getUserFavourites = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).populate({
      path: "favourites", // Populate favourites
      model: "Products", // Ensure "Products" is a valid registered model
    });
    ...
    return res.status(200).json(user.favourites); // Return populated favourites
  } catch (err) {
    console.error("Error fetching user favourites:", err); // Log error for debugging
    next(err);
  }
};